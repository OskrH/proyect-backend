...
router.post("/", async (req, res) => {
    try {
        ...
        console.log(carts);
        res.status(200).send("El carrito se creo con exito");
    } catch {
...
router.get("/:cid", async (req, res) => {
    try {
        ...
        if (!cartById) return res.send(`El carrito con el id ${cid} no existe`);

        res.status(200).send(cartById);
    } catch {
        return res.status(404).send(`el carrito con el ${cid} no existe`);
    }
});
...
router.post("/:cid/products/:pid", async (req, res) => {
    try {
        ...
        const cartById = carts.find((cart) => cart.id === parseInt(cid));
        if (!cartById)
            return res.status(404).send(`El carrito con el id ${cid} no existe`);
